...
```
                    ...
                    ├────────────────────────────┤
   type1 (logic) ──▶│ generate → judge flow over │   Qwen3-4B          ┐ concurrent
                    │ the resident vLLM line-up   │   Qwen3-4B-Instruct┘ generators
                    │                             │   Gemma-4-E4B       ← the JUDGE
   type2 (physics)─▶│ deterministic solver first  │   same generators + judge
                    │ then generate → judge       │   arbitrate the final answer/unit
                    └────────────┬───────────────┘
                                 ▼
...
- **Type 1 (logic)** — a *generate→judge* ensemble: two small generators
  (`Qwen/Qwen3-4B` + `Qwen/Qwen3-4B-Instruct-2507`) answer concurrently, then the
  **judge** (`google/gemma-4-E4B-it`, ~8B, thinking on) decides the correct answer
  and re-derives which premises were used. All three are **ungated** (no HF_TOKEN).
- **Type 2 (physics)** — a deterministic formula/template **solver** produces the
  first candidate answer + unit. The same two generators then answer with that
  deterministic result as reference only, and the same 8B judge solves
  independently before choosing the final answer/unit.

`serve/logic_config.yaml` is the shared serving LLM line-up for both types. The
filename is historical; there is no separate physics model config in `serve/`
because the serve folder is the single place that defines local vLLM model env,
ports, quantization, thinking mode, and residency swap.
...
5. [Choosing the shared resident line-up](#5-choosing-the-shared-resident-line-up)
...
│   ├── logic_config.yaml     shared Type 1/Type 2 model line-up (generators + judge)
│   ├── requirements.txt
...
│   │   ├── physics_adapter.py Type 2 solver-first generate→judge flow
...
| **vLLM (pinned)** | `setup.sh` installs **`vllm==0.19.1`** by default — a **CUDA-12** build that runs on drivers up to **CUDA 12.9** and supports the Qwen3 + Gemma-4 line-up. On a **CUDA-13** box (driver ≥ 580) set `VLLM_VERSION=` (empty) for the latest. |
| **`HF_TOKEN`** | **Optional** — the default line-up (`Qwen/Qwen3-4B`, `Qwen/Qwen3-4B-Instruct-2507`, `google/gemma-4-E4B-it`) is **ungated**. Only needed if you point a model at a gated repo. |
...
## 5. Choosing the shared resident line-up

The resident models live in **`serve/logic_config.yaml`**. The filename is
historical: this is the shared serve model config for both Type 1 logic and Type
2 physics. `setup.sh` launches **one vLLM server per model** listed there,
downloads only those models, and **refuses to start if the total exceeds
`max_resident_b`**.
...
```yaml
models:
  - id: Qwen/Qwen3-4B            # HF repo id (supports ${ENV:-default} substitution)
...
Top-level keys: `mode` (`arbiter` = Type 1 generate→judge, default; or `vote` =
weighted soft vote), `swap`, `quantization`, `thinking`, `max_resident_b`.
Type 2 physics uses the same `role: generator` and `role: judge` entries when
`PHYSICS_CASCADE=1`.

The default line-up: **`Qwen/Qwen3-4B` + `Qwen/Qwen3-4B-Instruct-2507`**
(generators) **+ `google/gemma-4-E4B-it`** (judge). Strictly-compliant
alternatives (2×4B, or a single ≤8B self-judge) are kept commented at the bottom
of the file — switching is a 30-second edit. See [§12](#12-compliance--the-8b-rule).

---

## 6. GPU sizing, swap & quantization

**Swap (`swap: true`, default) — RAM offload/reload.** The two 4B generators stay
co-resident and the 8B judge is **swapped in per query** via vLLM **sleep level 1**:
the inactive group's weights are offloaded from GPU to CPU RAM and copied back on
wake. Peak loaded model size is `max(4+4, 8) = 8B`, so the line-up fits the
momentary 8B design. The judge boots first (alone), is slept immediately, and the
generators load into the freed GPU memory; `VLLM_SERVER_DEV_MODE=1` enables the
`/sleep`, `/wake_up`, and `/is_sleeping` endpoints. Use `RESIDENCY_SLEEP_LEVEL=2`
only for non-FP8 experiments where discard/reload is acceptable.
...
| `LOGIC_CONFIG` | `serve/logic_config.yaml` | path to the shared Type 1/Type 2 line-up config |
...
| `RESIDENCY_SLEEP_LEVEL` | `1` | `1` = offload slept weights to CPU RAM; `2` = discard + reload from disk |
...
The swap uses vLLM **sleep level 1**: the slept group's weights are offloaded from
GPU to CPU RAM and copied back on wake. Only one group is awake on GPU at a time:

| Stage | On GPU (counts) | Asleep/off-GPU | GPU params |
|---|---|---|---|
| Stage 1 - generators | Qwen3-4B + Qwen3-4B-Instruct | judge | **8B** |
| Stage 2 - judge | the 8B Gemma-4-E4B judge | both generators | **8B** |

The same residency sequence is used by both Type 1 and Type 2. Type 2 runs the
deterministic physics solver first, then uses the generator phase and judge phase
above for LLM arbitration.

### RAM swap (sleep level 1)
Level 1 keeps slept weights in CPU RAM and removes them from GPU VRAM. It is the
default because it is fast and preserves FP8 weights. Level 2 discards and reloads
weights from disk; use it only for non-FP8 experiments where reload/re-quantization
is acceptable.

### The launch guard counts conservatively
`gateway/config.py` sums **total** params (16B for the default line-up) against
`max_resident_b` and logs a warning over 8B — a deliberately strict check.
`max_resident_b: 16` lets the default line-up launch; the GPU-resident peak is the
8B group size above. Strictly-compliant single-model alternatives are kept
commented in `serve/logic_config.yaml` if you want every declared model list to
stay ≤8B without explaining swap.
