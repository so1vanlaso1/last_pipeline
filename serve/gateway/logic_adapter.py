...
def _collect_votes(judges: List[Judge], record: Record) -> List[ModelReply]:
    ...
    for client, weight, cls, *_ in judges:
        try:
            raw = client.chat(system, user, max_tokens=512)
...
def _premises_used(
    ...
    try:
        data = primary.chat_json(system, user, max_tokens=128)
    except Exception:
        data = None
...
def _choice(judges: List[Judge], q: PredictQuery) -> PredictResult:
    ...
    if answer is None:
        answer = _choose_option_fallback(judges[0][0], q, options) or (
            options[find_uncertain_option(options) or 0] if options else (display or "Uncertain")
        )

    pu = _premises_used(judges[0][0], why, list(q.premises or []), q.query, answer)
...
def _choose_option_fallback(client: LLMClient, q: PredictQuery, options: List[str]) -> Optional[str]:
    ...
    user = f"Premises:\n{prem}\n\nQuestion: {q.query}\n\nOptions:\n{numbered}"
    try:
        data = client.chat_json(system, user, max_tokens=64)
    except Exception:
        data = None
...
def _think_tokens() -> int:
    return int(os.environ.get("LOGIC_THINK_TOKENS", "1024"))
...
def _generate_candidate(client: LLMClient, record: Record, persona: str) -> Dict[str, Any]:
    ...
    try:
        ...
        text = client.chat(system, user, max_tokens=_think_tokens())
    except Exception:
        text = ""
...
def _run_generators(gen_specs, record: Record) -> List[Dict[str, Any]]:
    with concurrent.futures.ThreadPoolExecutor(max_workers=len(gen_specs)) as ex:
        return list(ex.map(lambda s: _generate_candidate(s[0], record, s[1]), gen_specs))
...
def _arbiter_choice(gen_specs, arbiter: LLMClient, q: PredictQuery) -> PredictResult:
    ...
    with mgr.judge():
        try:
            text = arbiter.chat(system, user, max_tokens=_think_tokens())
        except Exception:
            text = ""
        data = extract_last_json_object(text) or {}
        ...
        if answer is None:
            answer = _choose_option_fallback(arbiter, q, options) or (
        ...
        if not pu:
            pu = _premises_used(arbiter, explanation, list(q.premises or []), q.query, answer)
    if not explanation:
...
def _arbiter_free_form(gen_specs, arbiter: LLMClient, q: PredictQuery) -> PredictResult:
    ...
    def gen(spec):
        c, persona = spec
        try:
            text = c.chat(gsys + persona, guser, max_tokens=_think_tokens())
        except Exception:
            text = ""
    ...
    with mgr.judge():                            # stage 2: swap the judge in
        try:
            text = arbiter.chat(asys, auser, max_tokens=_think_tokens())
        except Exception:
            text = ""
    data = extract_last_json_object(text) or {}
